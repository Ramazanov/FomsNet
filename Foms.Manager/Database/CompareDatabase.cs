    ...
    public static class CompareDatabase
    {
        ...
        public static void SaveDatabaseDiagramsInXml(bool pDestination, string pDatabaseName, SqlConnection pSqlConnection)
        {
            ...
            using(SqlDataReader reader = command.ExecuteReader())
            {
                ...
                while(reader.Read())
                {
                    string tableNameTemp = DatabaseHelper.GetString("table_name", reader);
                    if ((tableNameTemp != "sysdiagrams") && (tableNameTemp != "dtproperties") && (!tableNameTemp.StartsWith("Tconso_SP_Octopus_Consolidation_")))
                    {
                        ...
                        xml.WriteAttributeString("name",DatabaseHelper.GetString("column_name", reader));
                        xml.WriteAttributeString("type",DatabaseHelper.GetString("data_type", reader));
                        xml.WriteAttributeString("is_nullable",DatabaseHelper.GetString("is_nullable", reader).Trim().ToUpper());
